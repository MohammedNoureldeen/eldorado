...
import { OrderStatus, Platform, ProofType, UserRole, UserStatus } from '@prisma/client';
...
async function main() {
  ...
  const eldoradoOrderId = `db-smoke-${Date.now()}`;
  const created = await createOrder(workerActor, { eldoradoOrderId, platform: Platform.PC, coinQuantity: 100, grossSaleMinor: 1_000_000, saleCurrency: 'EGP' });
  await addCredentials(workerActor, created.id, { email: 'db-smoke@example.invalid', password: 'not-logged' });
  ...
  const provider = { getPrice: async () => ({ costMinor: 400_000, currency: 'EGP' }), createOrder: async () => ({ providerOrderId: 'db-smoke-provider', status: 'SUBMITTED', actualCostMinor: 420_000, currency: 'EGP' }), getStatus: async () => ({ providerOrderId: 'db-smoke-provider', status: 'COMPLETED', actualCostMinor: 420_000, currency: 'EGP' }), cancelOrder: async () => ({ status: 'CANCELLED' }), getBalance: async () => ({ balanceMinor: 1_000_000, currency: 'EGP' }) };
  ...
  await reconcileOrder(adminActor, created.id, { EGP: '1' });
